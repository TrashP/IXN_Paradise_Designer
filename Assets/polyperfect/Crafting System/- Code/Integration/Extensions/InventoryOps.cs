...
namespace Polyperfect.Crafting.Integration
{
    public static class InventoryOps
    {
        ...
        public static bool CanExtractCompletelyFromCollection(IEnumerable<IExtract<Quantity, ItemStack>> source, IEnumerable<ItemStack> toExtract)
        {
            ...
            {
                var leftover = i.Value - ExtractPossibleFromCollection(copy, i).Value;
                if (leftover > 0)
                    return false;
            }
        ...
        ///     If the extracted collection can be gleaned from the entirety of the given source.
        /// </summary>
        public static bool CanExtractCompletelyFromCollectionIncludingInstances(IEnumerable<IExtract<Quantity, ItemStack>> source, IEnumerable<ItemStack> toExtract)
        {
            var world = ItemWorldReference.Instance.World;
            var copy = source.Select(i => world.IsInstance(i.Peek().ID) ? new ItemStack(world.GetArchetypeFromInstance(i.Peek().ID), i.Peek().Value) : i.Peek()).ToList();
            foreach (var i in toExtract)
            {
                var leftover = i.Value - ExtractPossibleFromCollection(copy, i).Value;
                if (leftover > 0)
        ...
        {
            Profiler.BeginSample(nameof(ExtractCompletelyFromCollection));
            var list = new List<ItemStack>();
        ...
        /// </summary>
        /// <exception cref="Exception">If not everything can be extracted.</exception>
        public static List<ItemStack> ExtractCompletelyFromCollectionIncludingInstances(IEnumerable<IExtract<Quantity, ItemStack>> source, IEnumerable<ItemStack> toExtract)
        {
            Profiler.BeginSample(nameof(ExtractCompletelyFromCollectionIncludingInstances));
            var world = ItemWorldReference.Instance.World;
            var list = new List<ItemStack>();
            var enumeratedSource = source.ToArray();
            foreach (var extract in toExtract)
            {
                ...
                foreach (var sourceSlot in enumeratedSource)
                {
                    ...
                    if (sourceSlot.Peek().ID.Equals(totalExtracted.ID))
                        amount = remaining;
                    else if (world.IsInstance(sourceSlot.Peek().ID) && world.GetArchetypeFromInstance(sourceSlot.Peek().ID).Equals(totalExtracted.ID))
                        amount = 1;

                    var currentExtraction = sourceSlot.ExtractAmount(amount);
                    remaining -= currentExtraction.Value;
                    totalExtracted.Value += currentExtraction.Value;
        ...
        public static int TryExtractCount<T>(IEnumerable<T> source, int count) where T : IExtract<Quantity, ItemStack>
        {
            int extractedCount = 0;
            foreach (var extractFrom in source)
            {
                var extracted = extractFrom.ExtractAmount(count - extractedCount);

                extractedCount += extracted.Value;
